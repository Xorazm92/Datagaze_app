import { Injectable, NotFoundException, BadRequestException, UnprocessableEntityException, InternalServerErrorException } from '@nestjs/common';
import { InjectConnection } from 'nest-knexjs';
import { Knex } from 'knex';
import { ComputerDto, InstallApplicationDto, UpdateApplicationDto, RemoveApplicationDto } from './dto/computer.dto';
...
export class ComputersService {
  ...
  async getAllComputers() {
    try {
      const computers = await this.knex('computers')
  ...
  async installApplication(computerId: string, dto: InstallApplicationDto) {
    ...
    if (computer.status === 'inactive') {
      throw new BadRequestException('The selected computer is offline. Installation cannot proceed.');
    }
    ...
    try {
      ...
      if (!urlPattern.test(dto.installer_url)) {
        throw new UnprocessableEntityException('Invalid installer URL provided.');
      }
      ...
      throw new InternalServerErrorException('Failed to initiate installation. Please try again.');
    }
  }
  ...
  async updateApplication(computerId: string, dto: UpdateApplicationDto) {
    ...
    if (!installation) {
      throw new NotFoundException('Application not found on the target computer.');
    }
    ...
    try {
      ...
      if (!urlPattern.test(dto.update_url)) {
        throw new BadRequestException('Failed to download update from the provided URL.');
      }
      ...
      throw new InternalServerErrorException('Failed to initiate update. Please try again.');
    }
  }
  ...
  async removeApplication(computerId: string, dto: RemoveApplicationDto) {
    ...
    if (!installation) {
      throw new NotFoundException('The specified application was not found on the computer.');
    }
    ...
    try {
      ...
      throw new InternalServerErrorException('Failed to remove the application due to a system error.');
    }
  }
}
