...
export class ComputersService {
  ...
  private async logApplicationAction(computerId: string, appId: string, action: string, status: string, message?: string) {
    await this.knex('application_logs').insert({
    ...
    this.notificationGateway.emit('application_status', {
      computerId,
      appId,
      action,
      status,
  ...
  async installApplication(computerId: string, dto: InstallApplicationDto) {
    ...
    try {
      ...
      const [installation] = await this.knex('installations').insert({
        computer_id: computerId,
      ...
      await this.logApplicationAction(
        computerId,
        installation.id,
        'install',
        'started',
