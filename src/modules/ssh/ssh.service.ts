...
export class SSHService {
  ...
  async autoLogin(serverId: string, userId: string) {
    const credentials = await this.knex('ssh_credentials')
      .where({ server_id: serverId })
      .first();

    if (!credentials) {
      throw new NotFoundException('No stored credentials found for this server');
    }
    ...
    const serverData = {
      ...
      password: credentials.auth_type === 'password' ? this.decrypt(credentials.password) : undefined,
      private_key: credentials.auth_type === 'key' ? this.decrypt(credentials.private_key) : undefined,
      passphrase: credentials.passphrase ? this.decrypt(credentials.passphrase) : undefined
    };

    return this.connect(serverData, userId);
  }
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        conn.on('ready', () => {
          ...
          this.notificationGateway.emit('ssh_status', {
            type: 'connection',
            status: 'success',
